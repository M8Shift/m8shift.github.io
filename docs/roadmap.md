...
|---------|--------|--------------|
| **v3.50.1** <Badge type="tip" text="current" /> | 2026-07-04 | Hotfix for v3.50.0 dogfooding: long-lived relays can preserve an old `M8SHIFT.md` banner while their installed script is current. `update` now uses the best provable baseline among kit metadata, banner, and target script, so a stale banner no longer vetoes a supported manually promoted relay; truly pre-3.41 targets still refuse. |
