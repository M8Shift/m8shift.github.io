...
| 040 | [AI session usage monitoring](https://github.com/M8Shift/M8Shift/blob/main/docs/en/rfc/040-rfc-ai-session-usage-monitoring.md) | Usage snapshots + cooperative cooldown holds | **Phase 2 implemented** | v3.48.0 · 2026-07-04 | `usage init/adapters/snapshot/status` (read-only, fail-open) + `guard/watch/wait/resume` (advisory holds via core cooldown, no force, no auto-resume); Phase 3 open |
...
| 047 | [Headless liveness](https://github.com/M8Shift/M8Shift/blob/main/docs/en/rfc/047-rfc-headless-liveness-runner-listener.md) | Runner final-state enforcement + listener lifecycle | **implemented** | v3.46.0 + v3.47.0 · 2026-07-04 | Authorship-primary classification + `claim --refresh` (v3.46.0); listener lifecycle companion with OS backends, resume-working retry, 9 doctor findings (v3.47.0) |
| 048 | [Adoption discipline pack + local update](https://github.com/M8Shift/M8Shift/blob/main/docs/en/rfc/048-rfc-adoption-discipline-pack-update-health.md) | Agent pack + adoption health diagnostics + source-driven update | **implemented** | v3.49.0 + v3.50.0 + v3.50.1 · 2026-07-04 | `M8SHIFT.agent-pack.md`, compact anchor floor, adoption `doctor` findings, `update --target --source`, and stale-banner baseline hotfix for long-lived relays |
