...
|---|---|
| **Release (new tag)** | `roadmap.md` — new row at the TOP of release history (one-feature-per-line `<br>•` format, `<Badge type="tip" text="current" />` moved from the previous row) + the "On the design board" narrative bullet; `reference/rfc.md` — status/version columns of every RFC the release touched; `manifesto/technical-companion.md` — shipped-status rows; `security/audits.md` — if the release contained a security-relevant change or audit. |
